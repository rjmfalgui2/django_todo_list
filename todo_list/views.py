...
def home(request):
    return render(request, 'home.html', {'user': "rjmfalgui2"})

def about(request):
    my_name = "Reginald Juan Malagar Falgui II"
    return render(request, 'about.html', {"myname": my_name})

def contact(request):
    return render(request, 'contact-us.html', {'user': "rjmfalgui2"})
...
def listings(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            context = {'all_items': all_items, "user":"rjmfalgui2"}
